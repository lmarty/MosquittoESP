...
/* to make sure WebSockets::clientDisconnect(client,1002) works ... */
void WebSockets::clientDisconnect(WSclient_t * client, uint16_t code) {
  WebSockets::clientDisconnect(client,code,NULL,0);
}

/* explicit coding of all senFrame flavors */
bool WebSockets::sendFrame(WSclient_t * client, WSopcode_t opcode){
  return sendFrame(client,opcode,NULL,0,false,true,false);
}
bool WebSockets::sendFrame(WSclient_t * client, WSopcode_t opcode, bool mask){
  return sendFrame(client,opcode,NULL,0,mask,true,false);
}
bool WebSockets::sendFrame(WSclient_t * client, WSopcode_t opcode, uint8_t * payload, size_t length){
  return sendFrame(client,opcode,payload,length,false,true,false);
}
bool WebSockets::sendFrame(WSclient_t * client, WSopcode_t opcode, bool mask, bool fin, bool headerToPayload){
  return sendFrame(client,opcode,NULL,0,mask,fin,headerToPayload);
}
...
bool WebSockets::sendFrame(WSclient_t * client, WSopcode_t opcode, uint8_t * payload, size_t length, bool mask, bool fin, bool headerToPayload) {
    ...
    *headerPtr = 0x00;
    ...
    if(headerToPayload) {
        ...
        if(writeWS(client, &payloadPtr[(WEBSOCKETS_MAX_HEADER_SIZE - headerSize)], (length + headerSize)) != (length + headerSize)) {
            ret = false;
        }
    } else {
        // send header
        if(writeWS(client, &buffer[0], headerSize) != headerSize) {
            ret = false;
        }

        if(payloadPtr && length > 0) {
            // send payload
            if(writeWS(client, &payloadPtr[0], length) != length) {
                ret = false;
            }
        }
    }
...
void WebSockets::handleWebsocketCb(WSclient_t * client) {
    ...
    DEBUG_WEBSOCKETS("[WS][%d][handleWebsocket] ------- read message frame -------\n", client->num);
 ...
 * @return bytes send
 */
size_t WebSockets::writeWS(WSclient_t * client, uint8_t *out, size_t n) {
...
size_t WebSockets::writeWS(WSclient_t * client, const char *out) {
	if(client == NULL) return 0;
	if(out == NULL) return 0;
	return writeWS(client, (uint8_t*)out, strlen(out));
}
