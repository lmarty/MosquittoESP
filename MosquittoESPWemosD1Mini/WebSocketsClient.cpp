...
void WebSocketsClient::sendHeader(WSclient_t * client) {
    ...
    writeWS(client, (uint8_t*)handshake.c_str(), handshake.length());
    ...
    if(headerLine->length() > 0) {
        ...
        if(ok) {
			...
			if(clientIsConnected(client)) {
				writeWS(client, "This is a webSocket client!");
			}
			clientDisconnect(client);
		}
	}
...
int WebSocketsClient::connect(IPAddress ip, uint16_t port)
{
	if(!clientIsConnected(&_client))
		return 0;
	else
...
int WebSocketsClient::connect(const char *host, uint16_t port)
{
	if(!clientIsConnected(&_client))
		return 0;
	else
...
size_t WebSocketsClient::write(uint8_t c)
{
	return sendBIN(&c,1);
}
size_t WebSocketsClient::write(const uint8_t *buf, size_t size)
{
	DEBUG_WEBSOCKETS("[WS-Client] write dump");
	printType(buf[0]&0xF0);
	hexdump((uint8_t *)buf,size);
	return  sendBIN(buf,size);
}
int WebSocketsClient::available()
{
	if (_payload != NULL){
...
int WebSocketsClient::read()
{
	int c =  _payload[counter];
	counter++;
	return c;
}
int WebSocketsClient::read(uint8_t *buf, size_t size)
{
	for(int i=0; i<size;i++){
    if(i+counter>=_length) return (i+1);
    buf[i]=_payload[i+counter];
    counter++;
	}
 return size;
}
int WebSocketsClient::peek()
{
	return  _payload[counter];
}
...
uint8_t WebSocketsClient::connected()
{
	return  (clientIsConnected(&_client));
}
WebSocketsClient::operator bool()
{
	return  true;
}
void WebSocketsClient::setBuffer(uint8_t *payload, size_t length){
	counter = 0;
	_payload = payload;
	_length = length;
}
size_t WebSocketsClient::getBuffer(uint8_t *payload){
  payload = _payload;
  return _length;
}
void WebSocketsClient::printType(int type){
		String s;
		switch(type){
		case MQTTCONNECT:
			s = "MQTTCONNECT";
